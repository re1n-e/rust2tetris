...
impl SRLatch {
    ...
    fn update(&mut self, s: u8, r: u8) { // Changed to u8
        match (s, r) {
            (1, 0) => self.q = 1, // Set
            (0, 1) => self.q = 0, // Reset
    ...
    fn get_q(&self) -> u8 { // Changed to u8
        self.q
    }
}
...
impl DFlipFlop {
    ...
    pub fn set_initial_state(&mut self, initial_state: u8) { // Changed to u8
    ...
    pub fn d_flipflop(&mut self, d: u8, clk: u8) { // Changed to u8
    ...
    pub fn output(&self) -> u8 { // Changed to u8
        self.q
    }
}

#[derive(Clone, Copy)]
pub struct Bit { // 1 bit register
    dff: DFlipFlop,
}

impl Bit {
    pub fn new() -> Bit {
        Bit { dff: DFlipFlop::new() }
    }

    // Updates the register based on the input, load, and clock
    pub fn bit(&mut self, input: u8, load: u8, clk: u8) { // Changed to u8
    ...
    pub fn read(&self) -> u8 { // Changed to u8
        self.dff.output()
    }

    // Write a new value to the individual bit of the register
    pub fn write(&mut self, value: u8) { // Changed to u8
        self.dff.set_initial_state(value);
    }
}

pub struct Register { // 16 bit register
    bits: [Bit; 16],
}
...
impl Register {
    ...
    // Updates the register based on the input, load, and clock
    pub fn register(&mut self, input: [u8; 16], load: u8, clk: u8) { // Changed to u8
    ...
    pub fn output(&self) -> [u8; 16] { // Changed to u8
        let mut res = [0u8; 16]; // Changed to u8
        for i in 0..16 {
    ...
    pub fn read(&self, index: usize) -> u8 { // Changed to u8
        self.bits[index].read()
    }

    // Write a new value to an individual bit of the register
    pub fn write(&mut self, index: usize, value: u8) { // Changed to u8
